...
/** Pass `ticket` to edit it; leave it out to assign a new one. */
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => (next ? null : onClose())}>
      ...
      >
        ...
        {/* Mounted only while open, so each visit starts from the ticket as it
...
function Body({
  ...
  const [form, setForm] = React.useState(() => blank(ticket, projectId))
  const [pin, setPin] = React.useState<Pin | null>(
    ticket ? { lat: ticket.lat, lng: ticket.lng } : null
  )
  ...
  function submit() {
    ...
    mutation.mutate(parsed.data, {
      onSuccess: () => {
        toast.success(ticket ? "Ticket updated" : "Ticket assigned")
        onSaved?.()
        onClose()
      },
      onError: (error) =>
          ...
          {projects.isPending ? (
            ...
            <div className="border-a-400 bg-a-50 flex flex-col items-start gap-2.5 rounded-md border p-3.5">
              <p className="text-a-900 m-0 text-[13px] leading-relaxed">
                There are no projects yet, and every ticket belongs to one. Create
...
function blank(ticket?: TicketDTO, fallbackProjectId?: string) {
  return {
    title: ticket?.title ?? "",
    description: ticket?.description ?? "",
    projectId: ticket?.project?.id ?? fallbackProjectId ?? "",
    site: ticket?.site ?? "",
    radiusM: String(ticket?.radiusM ?? DEFAULT_RADIUS_M),
    startAt: toLocal(ticket?.startAt),
    endAt: toLocal(ticket?.endAt),
    assigneeIds: (ticket?.assignees ?? []).map((member) => member.id),
    priority: (ticket?.priority ?? "normal") as (typeof TICKET_PRIORITIES)[number],
  }
}
