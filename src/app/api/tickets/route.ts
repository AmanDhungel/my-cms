...
/**
 * Employees only ever see their own tickets — the filter is derived from the
...
export async function GET(request: NextRequest) {
  try {
    ...
    return ok({ tickets: tickets.map((ticket) => toTicketDTO(ticket, viewer.id)) })
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return ok({ ticket: toTicketDTO(ticket, viewer.id) }, 201)
  } catch (error) {
    return handleApiError(error)
  }
}
